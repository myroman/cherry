<div class="panel panel-default">
	<div class="panel-heading">
		<h4 class="panel-title"><a href="#collapse-shipping" class="accordion-toggle" data-toggle="collapse"
				data-parent="#accordion"><?php echo $heading_title; ?> <i class="fa fa-caret-down"></i></a></h4>
	</div>
						...
						success: function (json) {
							...
							if (json['error']) {
								...
								if (json['error']['parcelshopcity']) {
									$('select[name=\'parcelshopcity\']').after('<div class="text-danger">' +
										json['error']['parcelshopcity'] + '</div>');
								}

								if (json['error']['parcelshop']) {
									$('select[name=\'parcelshop\']').after('<div class="text-danger">' + json[
										'error']['parcelshop'] + '</div>');
								}
							}
							...
							if (json['shipping_method']) {
								...
								for (i in json['shipping_method']) {
									...
									if (!json['shipping_method'][i]['error']) {
										...
										html += '<div class="alert alert-danger">' + json['shipping_method'][i][
											'error'
										] + '</div>';
									}
								}
			...
			<script type="text/javascript">
				...
				$('select[name=\'parcelshopcity\']').on('change', function () {
					...
					$.ajax({
						...
						dataType: 'json',
						beforeSend: function () {
							$('select[name=\'parcelshopcity\']').after(
								' <i class="fa fa-circle-o-notch fa-spin"></i>');
						},
						complete: function () {
							$('.fa-spin').remove();
					...
					$.ajax({
						...
						success: function (json) {
							...
							function fillScheduleTable(data) {
								...
								var dayNames = ['Воскресенье', 'Понедельник', 'Вторник', 'Среда', 'Четверг',
									'Пятница', 'Суббота'
								];
								...
								function getHours(timeIntervals) {
									...
									for (i = 0; i < timeIntervals.length; i++) {
										...
										var timeStr = padWithHeadingZero(fromTime.Hours) + ':' +
											padWithHeadingZero(fromTime.Minutes) +
											'–' + padWithHeadingZero(toTime.Hours) + ':' + padWithHeadingZero(
												toTime.Minutes);
										times.push(timeStr);
									}
									return times.join(', ');
